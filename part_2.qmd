...
![](images/original_table.png "Original table"){width="208"}

![](images/coerced_table.png){fig-align="center" width="303"}
...
rand_cells <- tidyxl::xlsx_cells('data/Part_2/18_rand_numbers_harder.xlsx')
...
tidyxl::xlsx_formats(
  'data/Part_2/18_rand_numbers_harder.xlsx') |> 
  pluck('local','fill') |> 
  glimpse()
...
Albert put together three dataset to clean. The first two files are excel files. The third file is an csv file. The first two data sets describe the location of the warehouse, the product and the supplier id. The third data set provides information about the supplier itself (id, name, contact person, email, contract dates, and supplier rating. All three data sets are interconnected.
...
openxlsx::read.xlsx(
  'data/Part_2/19a_Shipments_Details_mod.xlsx') |> 
  as_tibble()
...
The headers added by tibble are not needed. We can use the r`ead.xlsx` function `startRow` = 2. Next we can use janitor to clean names. We can also use skimr library `skim()` function to request summary statistics.
...
skim reveals that inside the numeric part we have all of the dates (e.g., shipment, expected delivery, actual delivery and weight). However, all dates were transformed into numbers. They need to be recoded as dates.
...
Using `skim()` reveals that the mean of days_shipped_to_delivered is 19.5 days, ranging from 5 to 35 days.
...
The average days expected to actual delivery is 1.41, and the minimum is to expect the package to be delivered 3 days before. The highest delay is 7 days.
...
2.  We behead the first raw and end up with a column labeled `variable_type` after eliminating the label Warehouse. Using unpivotr reveals that the content of `variable_type` is the label `Warehouse`.
...
#* We need to coalesce or stich things together the columms
...
suppliers <- read_csv(
  'data/Part_2/19c_Suppliers_Info.csv') |> 
  janitor::clean_names()
