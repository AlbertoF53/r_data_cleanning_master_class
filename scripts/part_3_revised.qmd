...
The end result is 5 columns. Column \[,4\] we don't need because it includes the dot along the numbers from the email.

#### Just keeping the numbers column.

One way is to manipulate the parenthesis to group things together without including other characters. We can do so by including the question mark at the beginning of the group, followed by the column operator. `?:`
...
Basically we have two groups of string separated by a white space. Or `([a-z]+  ([a-z]+ )`. This option wont work since regex are case sensitive.
...
We need to alert R that names involve a collection of upper and lower letters
...
Correcting replacement. Remember that `\` is a reserved character in R. We could escape it using double `\\`
...
Notice the numbers are left as they are in the names. If we want to exclude those numbers we could expand the regular expression to capture the numbers associated to Otto Bauder 2 and Otto Bauder 21.

Remember that the regular expression creates three columns. The third column includes the number perse. By referring to columns 1 and 2, we only include first and last numbers.
...
### Match and exact number

While the `+` operator matches many things, sometimes you want an operator that matches an exact number of things. Such operator is particular useful when splitting data that has for example city abbreviations. For example matching a collection of exactly two.
...
#Step 3 Looking for exact matches
str_view(city_tib$city, "([A-Z]{2})")

#Step 4 Specifying range of matches
str_view(city_tib$city, "([A-Z]{2,3})")

```

### Using regexp to split the column inside the data set
...
#* Eliminating comma followed by space in the third column by taking
#* away the comma in the second group by using the question mark
#* followed by :
...
Without the optional `?` , eliminates the 4th entry composed of Berlin, with no state. It appears as `NA`. Allowing for `?` optional, Berlin entry is included, with NA for the state.

#### Creating a tibble after splitting out vector

And to get nice column names, we can us the .name_repair function
...
#### Separate wider option regex

Remember with `separate_wider_regex()` we are already doing the grouping eliminating the need of using parenthesis.
