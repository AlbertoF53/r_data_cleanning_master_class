...
Our first task is creating a dictionary for replacement. Passing the object template to extract the names using str_match_all() looking for things that are in brackets. However, we know that \[ \] are R special characters used for collections so that we need to escape them.

And in between these brackets, we can add many things such as `.+` , which allows us to collect letters. To get nicer results we need to add `unlist()` function.
...
However this overall `.+` approach is too lenient. It produces duplicates and unwanted texts. So instead of `.+`, we need to be more specific with our regular expressions. The regular expression allows to catch what we are looking for
...
Next we can use the unique function to avoid duplicates, which yields 11 unique fields.
