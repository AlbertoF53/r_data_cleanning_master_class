...
From demystifying regular expressions in R. Regular expressions can be used for:
...
There are two options for this transformation. Using the shortcut option in which there is only one reference to the column or variable to be changed. And the long version of several mutates for the same variable.
...
#     company = str_remove(company, ", "),
#     company = str_remove(company, ".com"),
#     company = str_remove(company, " Corporation"),
#     company = str_trim(company, side = c('both'))
#   )
...
### RegEx: Special collections of characters

Those include:

-   \[:lower:\]; \[:upper:\]; \[:letter:\]: \[:alpha:\]; \[:punct:\]; \[:graph:\]; `\d`;

```{r special_collections_1}
#| echo: true
#| warning: false

txt <-  "Hello I'm 10 years old."

#Displaying single lower cap letters
str_view(txt, '[a-z]')

#Displaying all groups of lower cap letters
str_view(txt, '[a-z]+')


```

#### Introducing the lower and upper special class operators

Notice that the `lower` operator needs to be preceded by `:` followed by `:`. There is also the `upper` class operator

```{r special_collections_2}
#| echo: true
#| warning: false

#Lower class operator
str_view(txt, '[:lower:]+')
...
#### Using alpha and letter

It brings a combination of capital and lower letters. Notice that only numbers are excluded from the search of characters. Both produce the same outcome.

```{r special_collection_alpha}
#| echo: true
#| warning: false
...
It used to single out or identify punctuation characters such as ", ',
...
Notice that so far these class operators do not catch numbers. The class `alnum` is applicable. It catches both letters and numbers.
...
The graph class combines all prior operators
...
`print` catches white spaces as well. It produces the same results as the `.` and `+` operators. If so, it is better to choose the dot and plus operators.
...
`\d` or d stands for numbers only

```{r searching _numbers_only}
#| echo: true
#| warning: false
...
You can invert the classes just by making the small letter into capital letter. For instance capitalizing `s` and `d`, means searching for anything that is not a number of a space

```{r inverting_characters}
#| echo: true
#| warning: false

#Inverting characters
str_view(txt, '\\D+')
...
MORE SPECIAL CASES CAN BE FOUND AT:

https://cran.r-projects.org/web/packages/stringr/vignettes/regular-expressions.htlml. Also consult regular expressions [regexp](https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html).

## Lesson 24

### RegEx: Find matches not preceded by something

Starting with this lesson our focus will on `lookarounds`. They are important to look for or to exclude specific things. For example, search for all cats but black.

Remember the `?:` for optional to identify the group that is unnamed. Now, we want to make the group `(black )` forbidden. The way to do it is to add the sign `<` to signify less of it followed by negating it `!`.

```{r unpreceded_matches_1}
#| echo: true
#| warning: false

text <- c("I saw a cat, a black cat, and a stray cat.")

#Looking for cat
str_view(text, 'cat')

#Excludying black cat from the search
#First attempt group black
str_view(text, '(black )cat')

#Recall using ?: before the group to signify optional group
str_view(text, '(?:black )cat')

#* To eliminate the group black use optional, followed by less of it, 
#* and then the negation of the object or group. ?<!

str_view(text, '(?<!black )cat')

```

Notice that it only catches the word cat. It does capture "I saw at.." , and "...a stray".

We can do the same for "stray cat". However, we could extend this option to capture `black` and `stray`. However, we need to form "black" and "stray" to be part of a group using `( )`, and qualifying this group using `?<!`. Or `?<!(stray|black).`

The regexpr `'(?<!(stray|black) )cat'` stands for only look for cats that are neither stray or black. The important thing here is that the operator `?<!(`

that handles the expression is the same. It consists of the question mark `?` that modifies the group. And then there is the "less than sign" `<` signifying that whenever we have a match we want to look behind the match; in this case "black" or "stray". And then we say we don't want `!` those two matches.

```{r unpreceded_matches_2}
#| echo: true
#| warning: false

#Excludying stray cat
str_view(text, '(?<!stray )cat')

#Exlcudying a varity of cats
str_view(text, '(?<!(stray|black) )cat')


```

##### Practicing these `lookaround` operators

Looking for things that are "not" cheap. We don't want words that the the `not` in them. We also want to get rid of the string "not really" cheap.

The next regexp means exclude not and any other character that range from 0 to 20 characters.

This regular expression `'(?<!not.{0,20} )cheap)'`
...
reviews <- c(
  ...
  "This product is not really cheap, but it is worth the price.",
  "Very cheap quality for the cost."
)

#Looking for strings cheap
str_view(reviews, 'cheap')

#Looking for things that are not cheap
str_view(reviews, '(?<!not )cheap')

#* Another possibility is to use { } with a number of spaces to 
#* capture other words interjected with not
str_view(reviews, '(?<!not.{0,20} )cheap')

#An alternative is 
str_view(reviews, '(?<!(not|not really) )cheap')
...
We want to exclude strings that are followed by something.

First search or view Otto Bauer. Next we want to exclude the numbers followed by the name.

```{r negative_lookahead_1}
#| echo: true
#| warning: false
...
#Looking for Otto Bauer
str_view(names, "Otto Bauer" )

#Trying to exclude the numbers using this regexp won't work
str_view(names,"Otto Bauer[ \\d]{0}")

#* Wrapping numeric \\d as a new group, and indicating we don't want
#* it
str_view(names, 'Otto Bauer(?![ \\d])')


```

Another example. We would be looking for the critical error messages. What about getting the errors that are not critical. We could use the `str_view(cols, pattern, match)` to identify strings that do not match the string `error critical`.

```{r negative_lookahead_2}
#| echo: true
#| warning: false

logs <- c(
  '2024-09-01 error critical: database failure',
  '2024-09-02 error warning: low disk space',
  '2024-09-03 error critical: server down',
  '2024-09-03 message info: A new version is available',
  '2024-09-04 error info: process completed'
)

#Singling out critical errors
str_view(logs,'error critical')

#Singling out errors that are not critical
str_view(logs, 'error critical', match = FALSE)

#* However this option displays the string `critical` as part
#* of the list.

#* The strategy of negative look aheads can be called upon. So that we
#* could look for the string `error` and then say don't take those
#* that are followed by white space and the word critical.
#* Grouping critical. Let's modify it so that it is a negative look
#* 

str_view(logs,'error(?! critical)' )

#Another option is to add the symbol for white space

str_view(logs,'error(?!\\scritical)' )

#* If you want to get particular messages, you could use str_match
#* to get strings not followd by critical but by other characters

str_match(logs, 'error(?! critical).+: .+')


#* If we want the error type and the message separately, we could
#* group these characters. In this case note we are grouping it 
#* together

str_match(logs, 'error(?! critical)(.+): (.+)')

#* Notice we have a white space in the fifth row " info", which we
#* don't want. We can include a space either blank or symbol \\s

str_match(logs, 'error(?! critical)\\s(.+): (.+)')


```

If you don't like the operators of question mark followed by #\* negation mark `?!` you could use chaining multiple string functions. For instance, you could use str_sub() listing the first string `error`. Next exclude, all the strings labeled `critical`, followed by the `negate = TRUE` option. This would eliminate all sentences or strings with the word `critical` in them.

```{r using_str_sub_chains}
#| echo: true
#| warning: false

logs |> 
  str_subset('error') |>
  str_subset('critical', negate = TRUE)
...
### RegEx: Positive Lookarounds

This lesson is the analog to the previous ones. In the previous lessons we excluded string that were either preceded or followed by some something. Now we want to include only things that are preceded or followed by something.

-   The first option we are looking for strings that follow the word apple

-   Second option is a variation of first option by specifying and then joining two groups of strings

-   Third option consists of two groups of strings. The first string specifies that the word we are looking for should be preceded by the word `apple` followed by a space. This yiels `pie`, `juice`, and `crumble`.

-   Fourth option produces identifies words joined with the word `pie`.

-   Fifth option identifies words that precede the word pie.

```{r possitive_lookaround_1}
#| echo: true
#| warning: false

text <- "apple pie, apple juice, apple crumble and plum pie"

#First option
str_view(text, 'apple\\s[a-z]+')

#Second option
str_view(text, '(apple\\s)[a-z]+')

#*Third option: include the word preceded by `apple`
str_view(text, '(?<=apple\\s)[a-z]+')

#Fourth option
str_view(text, '[a-z]+\\spie')

#Fifth option
str_view(text, '[a-z]+(?= pie)')
...
#### Selecting variables/columns using matches() along regex

A couple of small multiple examples to cover with regex.

The first is an exercise on patients' blood pressure across time.

1.  First eliminating patient_id using matches function. In this case `\\d`. This option also works in `pivot_longer()`
...
#Selecting everithing but patient id
bio_data |> 
  select(-patient_id) |> 
  view()

#Another option for selecting cases using matches()
bio_data |> 
  select(matches('bp_t|hr_t')) |> 
  view()

#Using regexp for selecting variable or columns with numbers
...
### Using regexp `\\d` with pivot() longer

In this example we want to to create a longer file whereby and flood testing and timing are separated in two distinct columns across time.

-   Under cols, we use the tidy helper matches along a regular expression to select the columns we want to widen. Notice we are using \\\\d which stands for digits and +. Or select columns containing one or more numbers in the name.

-   Under pattern, we identify the new two columns via regex. The first group starts with the letters bp or hr. The second group is preceded by the \_t and contains one or more numbers.

-   Names to instructs pivot longer about the names of the new two columns: pressure and time.

-   Notice that the columns time is reported as character. We could parse_number is using mutate. Alternatively , we could change it at the moment of doing the pivoting longer with the option names_transform = list(time = as.numeric).

```{r regexp_pivot_longer}
#| echo: true
#| message: false
...
bio_data |> 
  pivot_longer(
    cols = matches("\\d+"),
    names_pattern = '(bp|hr)_t(\\d+)',
    names_to = c("metric", "time"),
    values_to = 'val'
  )

#Parsing time as numeric using mutate
bio_data |> 
  pivot_longer(
    cols = matches("\\d+"),
    names_pattern = '(bp|hr)_t(\\d+)',
...
#Changing time as numeric within pivot longer
bio_data |> 
  pivot_longer(
    cols = matches("\\d+"),
    names_pattern = '(bp|hr)_t(\\d+)',
...
#### pivot_loner with sales data

Using another example of privot_longer with sales data. The purpose is to transform the database into 5 columns: id, type of transaction, month, year, and volume
...
sales_data <- tibble::tibble(
  id = 1:3,
  sales_jan_2021 = c(100, 150, 120),
  revenue_jan_2021 = c(1000, 1500, 1200),
  sales_feb_2021 = c(130, 140, 115),
  revenue_feb_2021 = c(1300, 1400, 1150),
  sales_jan_2022 = c(110, 160,125),
  revenue_jan_2022 = c(1100, 1600, 1250),
  sales_feb_2022 = c(140, 145, 135),
  revenue_feb_2022 = c(1400, 1450, 1350)
)

#Alberto's approach to create a new database
sales_data |> 
  pivot_longer(cols = -id,
               names_pattern = '(\\D+)_(\\D+)_(\\d+)',
               names_to = c("transaction", "month", "year"),
               names_transform = list(year = parse_number),
               values_to = "money")


#*Albert Rapp's approach. 
#*Notice in the names pattern. Albert approach is to use the two
#*labels sales or revenue using |, next group he uses `[a-z]+`
#*regex to capture month'. 
#*The third group is captured by specifying 4 numbers or
#* (\\d){4}

sales_data |> 
  pivot_longer(cols = -id,
               names_pattern = '(sales|revenue)_([a-z]+)_(\\d{4})',
               names_to = c("metric", "month", "year"),
               names_transform = list(year = parse_number),
               values_to = "val")
...
#### Selecting all columns related to sales for the year 2022

-   We can use a combination of tidy helpers sucha as contains('sales') as well as ends_with('2022'). However this combination yields all columns that have the word `sales` or the year `2022`.

-   The short path would involve using match() along the regexp sales followed by something then underscore 2022. Or 'sales\_.+\_2022'

```{r pivot_longer_2022}
#| echo: true
#| warning: false

#The first approach does not produce the desired solution
sales_data |> 
  select(contains("sales"), ends_with('2022'))

#Selecting sales that only took place in 2022
sales_data |> 
  select(contains("sales") & ends_with('2022'))

#Another short path
sales_data |> 
  select(matches('sales_.+_2022'))

```

#### Selecting columns penguins data

Selecting all columns that start with an `f` or a `b` and end with an `mm.`

Another option using regexp: At the start, `^` we want to have either f or b, followed by anything `.+` and at the end `$` we want \_mm.

regex along with tidy verse functions and helpers is a powerful combination.
...
#Or the long approach by combining several tidy functions

palmerpenguins::penguins |> 
  select(
    (starts_with('f') | starts_with('b')) & ends_with('_mm')  )

#Short solution with regexp
palmerpenguins::penguins |> 
  filter(!is.na(sex)) |> 
  select(matches('(f|b).+_mm'))

#Another regexp solution that brings a concise result
palmerpenguins::penguins |> 
  #We want to start the string and then we want to have the
  select(matches('^(b|f).+_mm$'))


```

### Big companies example

In this example, an illustration as to how to consolidate 6 changes done by str_remove() by one single regex.

Targeting company column, and using `str_remove_all( )` function. Once the characters have been removed, we can use str_trim() to remove unwanted spaces.

We can also replace International Business Machines by IBM

Next we can load the gt() package

```{r single_regex_for_six_changes}

big_tech_companies <- 
  readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-02-07/big_tech_companies.csv')

# Consolidating these 6 str_remove into a few regex
# big_tech_companies |> 
#   mutate(
#     company = company |> 
#       str_remove("Inc.") |> 
...
#       company, 
#       "Inc\\.|,|Corporation|\\.com|Systems|Platforms"),
...
clean_labels <- big_tech_companies |> 
  mutate(
    company = str_remove_all(
      company, 
      "Inc\\.|,|Corporation|\\.com|Systems|Platforms") |> 
...
#### Creating a gt table of the cleaned data set

We can add a function in the gt() procedure. The anonymous function goes as follows, whenever x gets an `_` it would be replaced it by a space. Next we use str_to_title() to create upper letters.
...
clean_labels |> 
  gt() |>
  cols_label_with(
    #Anonymous function whatever gets a _ replace it with space
    fn = function(x) x |> 
      str_replace_all('_', " ") |> 
      str_to_title()
  )
...
#### Reading and glueing a text

An example as to how to use regex along with tidy helpers to automate letters.
...
template <- read_file('data/Part_3/letter_template.txt') |> 
  glue::glue()
...
Our first task is creating a dictionary for replacement. Passing the object template to extract the names using str_match_all() looking for al things that are in brackets using the genexp `.+`. However, we know that \[ \] are R special characters used for collections so that we need to escape them.

And in between these brackets, we can add many things such as `.+` , which allows us to collect letters. To get nicer results we need to the add `unlist()` function.

```{r creating_dictionary}
#| echo: true
#| warning: false
...
However this overall `.+` approach is too lenient. It produces duplicates and unwanted texts. So instead of `.+`, we need to be more specific with our regular expressions. The regular expression belo allows to catch what we are looking for. Notice we adding the `+` to signify there are many capital and lower letters followed by space and a single underscore.

`[\\[[A-Za-z _]+\]`

```{r creating_dictionary_2}
#| echo: true
#| warning: false

template |> 
  str_match_all('\\[[A-Za-z _]+\\]') |>
  unlist()
  

```

However, this regular expression reports 19 fields, with several duplciates. Next we can use the unique function to avoid duplicates, which yields 11 unique fields. Having created a template, allows us to avoid typing the place holders one by one, which is a rather tedious task, prone to errors.

```{r creating_dictionary_unique}
#| echo: true
#| warning: false
...
The next step is to identify all 11 fields to be replaced.

##### Capturing content of fields

However, it would be too tedious to type the content of the 11 fields that need to be replaced by their corresponding regex. Using `str_escape()` allows us to identify the strings inside the brackets.

```{r capturing_content_str_escape}
#| echo: true
#| warning: false
...
Next, we can stick this patterns into a regular expression allowing to replace the text withihn the brackets for us.

##### Creating placeholder

Creating a placeholder

```{r creating_place_holder}
#| echo: true
#| warning: false
...
Creating data for place holder

It is an example of just one row of data across several columns. Using `slide()` can describe the contents of a row, and then `unlist()` to print the content of the case selected. Notice the data are organized by columns or variables.

##### Calling first case or row replacement dict.

```{r data_dictionary}
#| echo: true
#| warning: false

replacement_dict <- tibble(
  company_name = "Fake Comp",
  company_address = '4234 Wall Street',
  city_state_zip = 'New York City, NY, 11111',
  sender_phone_number = '111-111-1111',
  sender_email = 'anna@fake-comp.com',
  letter_date = 'Oct 01, 2024',
  first_and_last_name = 'John Customer',
  address = "San Diego, CA 22222",
  case_id = 'aa-bsa-25235234',
  contact_reason = 'a change of address',
  contact_date = 'Sep 12, 2024',
  sender_full_name = 'Anna Businessperson',
  sender_position = 'Customer Service Expert'
) |> 
  slice(1) |> 
  unlist()
...
##### Aligning replacement dict with placeholder

Using the `names()` function of the replacement dictionary to capture the names or content of the first row. Recall the vector replacement dictionary has the regex associated to each field in the letter associated with the information to be filled in the corresponding field.

We modify the replacement_dict vector with the regex contained in the placeholder vector. In this way the regex of each field is aligned with the data. In other words, the replacement dict has now the regex of the field associated with the data entry in the letter. Notice the NA associated with a a couple of fields or names. This means our regular expression did not capture all the elements of the letter.

```{r aligning_replacement_dict_placeholder}
#| echo: true
#| warning: false

names(replacement_dict) <- placeholder
...
Looking at the letter template, we discover that a place holder can also include commas `,`, which our regular expression failed to capture for example \[City, State ZIP Code\]. So adding comma to regex `('\[[A-Za-z _,]+\]')`.

```{r revising_regex}
#| echo: true
#| warning: false

#Capturing missing commmas
placeholder <- template |> 
  str_match_all('\\[[A-Za-z _,]+\\]') |>
  unlist() |> 
  unique() |> 
...
#Changing replacement dictionary with revised placeholder
names(replacement_dict) <- placeholder 
...
Notice the revised place holder took care of the missing comma, bringing back the missing field \[City, State, ZIP Code\]; however, it missed one field or stakeholder since there is an NA.

The problem can be traced back to Case ID field \[ID1\], which includes digits or numbers along letters.

```{r correcting_reason_contact}
#| echo: true
#| warning: false

#* Capturing missing CASE ID [ID1] field to include numbers

placeholder <- template |> 
  str_match_all('\\[[A-Za-z _,\\d]+\\]') |>
  unlist() |> 
  unique() |> 
  str_escape()
...
Another thing that is wrong is the field \[Today's Date\], since the regular expression misses the possessive symbol '.

```{r adding_missing_possesive_date}
#| echo: true
#| warning: false

#* Capturing missing apostrophe in [Today's Date] field
...
Adding the ' to capture the field \[Today's Date\] produces an error message.

"Error in names (replacement_dict) \<- placeholder:

'names`attribute [14] most be the same length as the vector [13]`

Basically this mean that we have two fields with the Name in it: "\\\[First Name\\\]" and "\\\[Last Name\\\]". The problem can be traced back to the original replacement_dict tibble data were first and last names are combined as follows \`first_and_last_name =

##### Overriding placing holders

We can combine the two fields by overriding the placeholder vector, using the original place holder by skipping 1 thru 6 entry, and using the paste function to combine the 7th and 8th entry, corresponding to first and last name. Next we skip the first to the 8th place holder eliminating the 14th entry.

```{r combining_names_overriding}
#| echo: true
#| warning: false

placeholder <- c(
  placeholder[1:6],
  paste(placeholder[7],placeholder[8]),
  placeholder[-(1:8)]
)
...
However, I was not able to replicate Albert Rapp's solution. Consequently, I focused my attention to correcting the replacement_dict tibble file itself. The purpose was to align the placeholder to the original template of the letter. Accordingly, I replaced the `first_and_last_name = 'John Customer'` in the tibble file by the original `[First Name]  = John` and `[Last Name] = Customer]` as originally envisioned in the template. I avoided the complicated procedure of replacing the placeholder.

##### Aligning tibble to original template of a customer letter
...
replacement_dict <- tibble(
  ...
  sender_email = 'anna@fake-comp.com',
  letter_date = 'Oct 01, 2024',
  #Adding field for first name
  first_name = 'John',
  #Adding field for last name
  last_name = 'Customer',
  address = "San Diego, CA 22222",
  case_id = 'aa-bsa-25235234',
  contact_reason = 'a change of address',
...
#Applying template of genex to template of letter generate placeholder
...
#Displaying new replacement dictionary
replacement_dict

```

Replacing replacing the template with the replacement dictionary and gluing the sentences together.

```{r alternative_solution_2}

template |> 
  str_replace_all(replacement_dict) |> 
  glue()
