...
read_csv("/Users/albertocabrera/Documents/GitHub/r_data_cleanning_master_class/data/Part_2/05_exibble_with_metadata.csv",skip = 2)
```
...
One package useful for reading excel files is {`openxls}`. However, the data frame looks weird. This is why we transform it into tibble with the following function `as_tibble()`.
...
Notice variables missing_Null and missing_dates, or columns, are incorrectly identified as characters.
...
You can actually tell `read_xlsx()` what constitutes a missing value. With the `na.strings` argument, you can specify a vector of strings that should be treated as `NA`. In this case na.strings = c(" ", "(null)", "Unknown") instructs read.xlsx to treat empty spaces, (null) and Unknown as missing.
...
openxlsx::read.xlsx(
  ...
  mutate(
    missing_NULL = parse_number(
      missing_NULL,
      #Notice parse_function allows to report missing values
      na = c("", "NA", "(null)", "Unknown")),
    missing_dates = 
...
This lesson introduces the equivalent concepts of merging data sets from Stata and SPSS
...
## Lesson 12: Combining data sets by left\_ and right_join

This lesson illustrates how to combine two data sets. Each data set has different rows, and different names for the key variable. In the flights data set the key variable is `origin`. In the airports data set, the name of the key variable is `faa`.
...
Selecting some variables from the flights data set. We want to add the name of the carrier. They key matching variable in both data sets is `carrier`.
...
#### Another example of left join not as easier as the previous one 

In this example we have abbreviations for airport destinations. For merging, we need another database with the full name of the airports. This name is found in the `airports` data set under the label `faa`.

In the example below, we are filtering from the airports' column faa the names that appear in the flights data set variable origin. In other words the two variables are the same (faa = origin) but have different labels. The trick is to eliminate duplicates.
...
#* which are located in the airports data set
...
We get exactly the same data set made up of 435,352 rows. The reason it produced the same results is the filtering we applied to the airports data set `filter(faa %in% unique(flights$origin)).`
...
Let's see what happens when eliminating the filtering that allowed both data sets to have the same airports.
...
Other joins being illustrated with a small data set. In this example the left_join kept all the airports from the `flights` data set, or 1,251. But the columns are empty except for the New York airports that are contained in the `flights` data set.
...
Using two rows, or two rows per airport.
...
openxlsx::read.xlsx(
  ...
  mutate(
    across(.cols = contains('date'),
           .fns = function(x) lubridate::as_date(x))
  )
...
Moving to the supplier information data set. While the data set was loaded without a problem, the column names need to be cleaned.
